...
export class HeaderComponent implements OnInit {
  ...
  // DMS notifications (uploads under Regulation Setup / Announcements /
  // Organization, plus DMS file/folder shares). This is the only feed the
  // header bell shows — the admin approval-notifications fields above
  // (notifications/unreadCount/etc.) are kept for their own modal but aren't
  // wired into the bell/popup.
  ...
  ngOnInit() {
    ...
    // Badge refreshes on load, then every 60s (DmsNotificationService.startPolling).
    this.dmsNotificationService.unreadCount$.subscribe(count => this.dmsUnreadCount = count);
    this.dmsNotificationService.startPolling();
  }
  ...
  toggleNotificationPopup() {
    ...
    if (this.showNotificationPopup) {
      ...
      this.dmsNotificationService.getNotifications(false).subscribe({
        next: (summary) => {
          this.dmsNotifications = summary.notifications || [];
          this.dmsUnreadCount = summary.unreadCount || 0;
  ...
  // Regulation Setup / Announcements / Organization, and DMS file/folder
  // share notifications. Badge polls every 60s (DmsNotificationService);
  // the list itself refreshes when the popup opens and updates locally
  ...
  private markDmsNotificationReadLocally(notification: DmsNotificationResponse): void {
    ...
    this.dmsNotificationService.markAsRead(notification.id).subscribe({
      next: (result) => {
  ...
  markAllDmsNotificationsRead(event: Event): void {
    event.stopPropagation();
    if (this.dmsUnreadCount === 0 || this.isMarkingAllDmsRead) {
      return;
    }
    this.isMarkingAllDmsRead = true;
    this.dmsNotificationService.markAllAsRead().subscribe({
      next: (result) => {
        if (result?.success !== false) {
   ...
   * resolvePendingDeepLink()/tryResolvePendingDeepLink() there.
   */
