...
export class UserApprovalComponent implements OnInit {
		...
		{ headerName: 'Approved By', field: 'approvedBy', width: 150 },
		{
			...
			cellRendererParams: {
				clicked: (field: any) => {
					...
					if (field.data.approvalType == 'User' && field.data.createdBy != this.persistance.getUserId() && !this.persistance.isUser()) {
						this.openUserReviewPopUp(this.pendingApprovalModel!)
					}
					else if (field.data.approvalType == 'Access' && field.data.createdBy != this.persistance.getUserId() && !this.persistance.isUser()) {
	...
	getAllUserPendingApproval() {
		this.rowData = [];
		this.apiService.getAllUsersPendingApprovals(this.persistance.getUserUID()!).subscribe((result: any) => {
